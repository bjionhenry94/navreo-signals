-- analytics_hub_v3 (2026-08-02, panel round-1 fix BE-2/FE-1): __all must be the
-- WHOLE book. The v2 rep/mtg/wk CTEs inner-joined replies to campaign_scorecard,
-- and subsequence campaigns (Interested Reply / Meeting Request flows) are
-- deliberately absent from the scorecard — their replies and booked meetings
-- fell out of '__all' entirely (measured 30d: 44 replies + 3 meetings dropped;
-- hub said 11 meetings while perf_daily_series_v2 said 14 for the same window).
-- v3 LEFT-joins and coalesces the missing label to '__unassigned', so
-- subsequence activity counts in '__all' (and the excluded-from-display
-- '__unassigned' bucket) without polluting any named client.
-- Function name stays analytics_hub_v1 (the server calls it by name).

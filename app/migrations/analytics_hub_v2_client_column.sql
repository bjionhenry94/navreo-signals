-- analytics_hub_v2 (2026-08-02, platform-wide-stabilise): the hub RPC no longer
-- carries its own client-attribution keyword copy. campaign_scorecard.client is
-- THE label — written by _client_win_label in server.py (the single authority,
-- same one client-windows uses) on every scorecard sync, backfilled 2026-08-02.
-- Unmatched shared-workspace campaigns are '__unassigned': they count in '__all'
-- (the whole book) but never appear as, or inside, a named client — the old
-- CASE fell back to 'Navreo' and inflated it (documented ~34% on 30d sent).

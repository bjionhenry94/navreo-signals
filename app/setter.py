...
def draft_reply(reply: dict, agent: dict, classification: dict, slots: list, slot_status: str, sender_first: str,
    ...
    payload["constraint_directive"] = (
        ...
        "the slots you were given, and MUST NOT include the 'Would you be open to a call on ...' "
        "paragraph, the 'If those times aren't suitable' line, or any booking-link/availability "
        "ask - drop those paragraphs entirely, even where a template says to keep every block as "
        "written. Instead: answer their actual message, acknowledge the constraint in one natural "
        "sentence and defer to it ('I'll circle back in January as you suggested', 'I'll grab a "
        "time on your calendar for after Oct 2'). When the lead pointed at their OWN calendar or "
        "window, commit to that - link their calendar, never ours. When a call is already booked "
        "or a proposed time is being accepted, confirm that exact time and add nothing new. When "

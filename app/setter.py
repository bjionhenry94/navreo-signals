...
- When this draft CONFIRMS a call - a time is agreed, they booked, or the lead gave their own booking link for us to use - the confirmation always has two parts (owner rule 2026-08-16, every agent, every campaign): state the booking as "I've booked in for [day and time]" - write the real day and time when the thread has already fixed them, otherwise leave the bracketed placeholder exactly as [day and time] for the reviewer to fill in after booking - and always close the confirmation with "You will be speaking with [name]." - leave [name] as a bracketed placeholder unless the thread itself makes plain who will be on the call.
...
def draft_reply(reply: dict, agent: dict, classification: dict, slots: list, slot_status: str, sender_first: str,
    ...
    payload["constraint_directive"] = (
        ...
        "sentence and defer to it ('I'll circle back in January as you suggested'). When the "
        "lead gave their OWN booking link, the reviewer books before sending: write \"I've "
        "booked in for [day and time]\" (leave the bracketed placeholder as-is) and close with "
        "\"You will be speaking with [name].\" - their calendar, never ours. When a call is "
        "already booked or a proposed time is being accepted, confirm that exact time, close "
        "with the same \"You will be speaking with [name].\" line, and add nothing new. When "

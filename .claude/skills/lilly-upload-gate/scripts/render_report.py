#!/usr/bin/env python3
"""Render a lilly-upload-gate run JSON as an HTML review page in the Navreo signals
design system. Serve with serve_review.py: every flag is actionable on the page —
one-click Fix where the correct value is known, inline inputs for missing values,
Drop lead, and bulk Override with a mandatory reason. Decisions persist to
<run>.decisions.json; the corrected upload list is served at /api/rows.

Usage: render_report.py <run_result.json> [output.html]   (static, read-only render)
"""
...
PAGE_SIZE = 5
FONT = pathlib.Path.home() / "navreo-signals/app/fonts/AcidGrotesk-Normal.otf"
...
def gate_state(d, decisions):
    ...
    open_fail = any(st == "open" and d["results"].get(d["flags"][i]["check"], {}).get("status", "FAIL") == "FAIL"
...
def render(d, decisions=None, live=False, list_id=None):
    d = normalise_run(d)
...
<style>{CSS.replace('__FONT__', FONT.as_uri() if FONT.exists() else '')}</style></head><body>
<div class="main">
  <div class="pagehead">
    <div>
...
if __name__ == "__main__":
    ...
    decf = src.with_suffix(".decisions.json")
    decisions = json.loads(decf.read_text()) if decf.exists() else []
    stamp = d["run_at"][:19].replace(":", "-")
    default = pathlib.Path.home() / f".claude/skills/lilly-upload-gate/runs/{stamp}-{d['campaign']['id']}.html"
    out = pathlib.Path(sys.argv[2]) if len(sys.argv) > 2 else default
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(render(d, decisions, live=False))
    print(out)

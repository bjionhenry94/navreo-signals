...
## Lead-variable fill rate (FAIL or WARN — runs in Step 5b)

Runs as `scripts/check_lead_variable_fill.py` against the live campaign — fetches every lead and verifies that each custom `{{variable}}` referenced in the copy is populated on every lead.

| Check | Severity | Why |
|---|---|---|
| Custom variable filled on 100% (or ≥99.9%) of leads | PASS | Copy will render correctly for every lead |
| Custom variable filled on 95–99.9% of leads | WARN | Small gap — likely stragglers from a partial backfill or late lead additions |
| Custom variable filled on < 95% of leads | FAIL | Meaningful share of leads will send copy with a blank where the variable should be — broken sentence + deliverability damage |
| Variable's value found under a different-cased key (e.g. `cold email video angle` vs `Cold Email Video Angle`) | FAIL | Smartlead's `{{var}}` lookup is case-sensitive — these render empty unless keys are normalised |

Default lead fields (`first_name`, `last_name`, `email`, `company_name`, `title`, etc.) are skipped — those are top-level on the lead object, not in `custom_fields`. Smartlead built-ins like `{{sl_date "..."}}` are also skipped.

If any custom variable is below 100%, the report should hold the user back from launching until they either (a) backfill the missing leads via `lilly-personalisation`, (b) remove the variable from copy, or (c) confirm the unfilled leads are acceptable losses.
